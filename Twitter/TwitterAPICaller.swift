...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func postTweet(tweetString: String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let tweet_url = "https://api.twitter.com/1.1/statuses/update.json"
        TwitterAPICaller.client?.post(tweet_url, parameters: ["status": tweetString], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func favoriteTweet(tweetID: Int,  success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let fav_url = "https://api.twitter.com/1.1/favorites/create.json"
        TwitterAPICaller.client?.post(fav_url, parameters: ["id": tweetID], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func unfavoriteTweet(tweetID: Int,  success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let fav_url = "https://api.twitter.com/1.1/favorites/destroy.json"
        TwitterAPICaller.client?.post(fav_url, parameters: ["id": tweetID], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
    ...
    func retweet(tweetID: Int,  success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let rt_url = "https://api.twitter.com/1.1/statuses/retweet/\(tweetID).json"
        TwitterAPICaller.client?.post(rt_url, parameters: ["id":tweetID], progress: nil, success: { (task:URLSessionDataTask, respone: Any?) in success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
